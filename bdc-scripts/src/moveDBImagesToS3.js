...
const { moveInspectionImagesToS3, getInspectionList } = require('./bdc_api');
...
async function callMoveInspectionImages(BASE_URL, inspectionId, doneCB) {
    let resp = await moveInspectionImagesToS3(BASE_URL, inspectionId);
    doneCB(inspectionId, resp);
}

async function moveAllInspections() {
    let inspectionList = await getInspectionList(BASE_URL);
    console.log(`Will move images from ${inspectionList.length} inspections...`);
    ...
    for (let inspection of inspectionList) {
        callList.push(moveInspectionFilesLimiter.schedule(() => 
            callMoveInspectionImages(BASE_URL, inspection.id, doneUploadCB))
            .then((resp) => {})
...
if (process.argv.length < 3) {
    ...
    console.log.apply('Usage: node ./src/moveDBImagesToS3.js hostname');
    process.exit(1);
}
