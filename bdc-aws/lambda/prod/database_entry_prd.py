...
def processedFileName(s3_object_key, prefix):
...
def getBladeFolderName(s3_object_key, bucket):
    ...
    try:
        ...
        for record in event['Records']:
            ...
            if s3_object_key.startswith(cv_pipeline_input):
                folder_name, total_size = getBladeFolderName(s3_object_key, source_bucket)    # Split the path into folders
            ...
            if s3_object_key.startswith(cv_pipeline_output):    
                ...
                blade_name =  processedFileName(s3_object_key, cv_pipeline_output)
