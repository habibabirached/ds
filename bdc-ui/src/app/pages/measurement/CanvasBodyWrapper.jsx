...
const CanvasBodyWrapper = ({
  ...
  const onAnnotationUpdate = (data) => {
    console.log('onAnnotationUpdate() called with:',data)
    if (data.polygons != null) {
      for (let polygon of data.polygons) {
        if (polygon.category != null)
          console.log('label:',polygon.category);
          onCategoryUpdate(polygon.category);
      }
    }
  };
  ...
  const {
    ...
    setDistance,
    grayOutSaveButton,
  } = useCustomStateHook(inspectionId, false, onAnnotationUpdate);
